...
//
// Originally included in vs_library, but this is too niche, and it shouldn't be in the library.
//
//------------------------------
//
// Initiating:
...
{
	if( !Ent("vs_think_console") )
		THINK <- VS.CreateTimer( "vs_think_console", freq )
...
// @param Vector [ 0,0 : top left corner : position in world space ]
// @param float [ distance between 2 cells/pixels ]
// @param integer [ the index of the array, storing the position ]
// @param string [ "xy" / "yz" ]
function Console::SetPositions( worldstart, distance, idx, plane = "yz" )
{
	...
	if( !("GFX" in ::VS.Console) ) throw "Create the graphics array first with: Console.CreateDisplay(X,Y,ix)"
...
// Spawning props use too much resources.
